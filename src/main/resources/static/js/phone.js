$(function () {
  ...
  $('#examplePhone').blur(function () {
    let phone = $('#examplePhone').val()
    let phoneRegExp = /^1([3456789])\d{9}$/
    let ok = phoneRegExp.test(phone)
    if (!ok) {
      $('#phoneError').text('手机号码为空或不合法').css('color', 'red')
    } else {
      $('#phoneError').text('')
    }
  })
...
$(function () {
  //验证码发送ajax
  let yzm = () => {
    const phoneValue = $('#examplePhone').val()
    if (phoneValue) {
      $.ajax({
        type: 'POST',
        url: '/sendCode',
        data: {number: phoneValue, type: 'phone'},
